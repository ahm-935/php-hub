...
if (isset($_POST['upload'])) {
    ...
    if(!file_exists($file['tmp_name'])){
        $msg = "<span style='color:red'>Please select a image file.</span>";
    }
    elseif($file['size'] > (500 * 1024)) {
        $msg = "<span style='color:red'>Image size should be less than 500kb.</span>";}
     elseif(($type == "image/png" ||
        $type == "image/jpeg" ||
        $type == "image/jpg"
       ) == false) {
        $msg = "<h3><span style='color:red'>Invalid file type. please upload .png, .jpg, .jpeg, file</h3> </span>";
    }
     else {
        $msg = "<span style='color:green'>Image uploaded successfully.</span>";
        move_uploaded_file($file['tmp_name'], $final_path);
        $img = "<img src='$final_path' width='500'>";
    }
